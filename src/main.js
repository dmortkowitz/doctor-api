...
$(document).ready(function() {
  ...
  $(".docForm").submit(function(event) {
    ...
    let searchTerm = document.getElementById("doc-form").value;
    // $('#doc-form').val();
    let newDoctor = new Doctor();
    ...
    if(body.data.length === 0){
      ...
      for (let i = 0; i<body.data.length; i++) {
        $('.results').append(`<h2><li>${body.data[i].profile.first_name}</li> `+`${body.data[i].profile.last_name}`+`<br></h2><br><h4>Gender: </h4> ${body.data[i].profile.gender}`+` <br> <h4>Phone number:</h4>
  ...
  $(".sickForm").submit(function(event) {
    ...
    let searchAil = document.getElementById("#sick-form").value;
    let newDoctor = new Doctor();
